...
export class FavouriteService {
  ...
  async addToFavourites(accountId: string, componentId: string): Promise<Favourite> {
    // Kiểm tra xem đã tồn tại chưa
    const existing = await this.favouriteModel.findOne({ accountId, componentId });
    if (existing) {
  ...
  async removeFromFavourites(accountId: string, componentId: string): Promise<void> {
    const result = await this.favouriteModel.deleteOne({ accountId, componentId });
  ...
  async toggleFavourite(accountId: string, componentId: string): Promise<{ 
    isFavourite: boolean; 
    message: string 
  }> {
    const existing = await this.favouriteModel.findOne({ accountId, componentId });
  ...
  async getFavouritesByAccount(accountId: string): Promise<Favourite[]> {
    return this.favouriteModel.find({ accountId }).sort({ createdAt: -1 }).exec();
  }

  // Kiểm tra component có trong favourites không
  async isFavourite(accountId: string, componentId: string): Promise<boolean> {
    const favourite = await this.favouriteModel.findOne({ accountId, componentId });
    return !!favourite;
  }
